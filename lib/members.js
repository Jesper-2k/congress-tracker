...
// Shared by getPositiveTickers() and getInferredPortfolio() below: sums buy
// midpoints minus sell midpoints per ticker. Split out so the page can ask
// "which tickers have a positive position" *before* fetching any prices —
// see getPositiveTickers().
...
// Tickers with a positive estimated net position — i.e. the ones actually
// worth a live price lookup. Callers should compute this *before* calling
// lib/yahooFinance.js's getCurrentPrices(), so tickers the member has fully
// exited (net position <= 0) never cost a Yahoo Finance call at all instead
// of being fetched and then discarded by getInferredPortfolio() below.
export function getPositiveTickers(trades) {
  const byTicker = computeNetPositions(trades);
